...
def load():
    import pandas as pd
    file_path = r"C:\Users\Elif\Desktop\bootcamp2\TelcoChurn\Telco-Customer-Churn.csv"
...
df["TotalCharges"].fillna(df["MonthlyCharges"]*df['tenure'], inplace=True)# aylık ödenecek miktarlarıyla totalcharge doldurulailir (daya iyi olur denensin)  veya 11 değişken drop edilebilir
df[num_cols]
df.isnull().sum()
...
for name, model in models:
    cv_results = cross_validate(model.fit(X,y), X, y, cv=10, scoring=["accuracy", "f1", "roc_auc", "precision", "recall"])
    print(f"########## {name} ##########")
...
def check_outlier(dataframe, col_name):
    ...
    if dataframe[(dataframe[col_name] > up_limit) | (dataframe[col_name] < low_limit)].any(axis=None):
        ...
        return False
for col in num_cols:
    print(outlier_thresholds(df,col),col)
...
df["NEW_FLAG_AutoPayment"] = df["PaymentMethod"].apply(lambda x: 1 if x in ['Credit card (automatic)','Bank transfer (automatic'] else 0)
...
catt,numm,carr=grab_col_names(df)
from sklearn.preprocessing import MinMaxScaler, LabelEncoder, StandardScaler, RobustScaler
rare_analyser(df,'Churn',catt)
for col in numm:
