...
# 1. iyzico_data.csv dosyasını okutunuz. transaction_date değişkeninin tipini date'e çeviriniz.
df = pd.read_csv(r"C:\Users\Elif\Desktop\iyzico_data.csv")
...
for id in df.merchant_id.unique():
    ...
    a=df[(df.merchant_id == id) & ( df.transaction_date >= "2018-01-01" ) & (df.transaction_date < "2019-01-01")]["Total_Transaction"]
    print(a)
    a.plot()
    plt.xlabel('')
...
df.sort_values(by=['merchant_id','transaction_date'],inplace=True)
###############################################################
...
new=pd.DataFrame({'a':df.groupby(["merchant_id"])['Total_Transaction'].transform(lambda x: x.shift(90)).values,'b':df['Total_Transaction'].values})
new.head(50)
...
########################
#manuel black-friday calcculation code :)
def get_black_friday_dates(start_year, end_year):
    ...
    for year in range(start_year, end_year + 1):
        ...
        black_friday = fourth_thursday + pd.DateOffset(days=1)
...
for i in df['merchant_id'].unique():
      merc=df.loc[df['merchant_id']==i,["Total_Transaction",'transaction_date']]

      plt.plot(merc['transaction_date'],merc['Total_Transaction'])
      plt.title('Merchant - {}'.format(i))
      plt.show(block=True)
